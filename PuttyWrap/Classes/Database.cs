...
namespace PuttyWrap.Classes
{
	...
	public class Database
	{
		...
		public SQLiteConnection Open()
		{
        	...
        	//string db_filename = Application.StartupPath + "\\PuttyWrap.db3";
            string db_filename = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\PuttyWrap.db3";
		...
		private void UpgradeSchema()
		{
			...
			switch (GetKey("db_version"))
			{
				case "1.0.1":
                    ...
                    string puttyExe = @"\\ms\dist\fsf\PROJ\putty\0.62\exec\putty.exe";
                    if (File.Exists(puttyExe))
                    {
					    SetKey("putty_exe", puttyExe);
                    }

                    string pscpExe = @"\\ms\dist\fsf\PROJ\putty\0.62\exec\pscp.exe";
		...
		{
            RegistryKey key = Registry.CurrentUser.OpenSubKey(@"Software\Morgan Stanley\PuttyWrap\Settings");
            if (key != null)
            {
