...
namespace PuttyWrap
{
    public partial class frmPuttyWrap : Form
    {
        ...
        public frmPuttyWrap(string[] args)
        {
        ...
        private void dockPanel1_ActiveDocumentChanged(object sender, EventArgs e)
        {
            if (dockPanel1.ActiveDocument is ctlPuttyPanel)
            {
	            ...
	            this.Text = p.ApplicationTitle.Replace(" - PuTTY", "") + " - PuttyWrap";
				p.Text = p.ApplicationTitle.Replace(" - PuTTY", "");
                p.SetFocusToChildApplication();
            }
        }

        private void frmPuttyWrap_Activated(object sender, EventArgs e)
        {
            //dockPanel1_ActiveDocumentChanged(null, null);
        }

        private void aboutPuttyWrapToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void PuttyWrapWebsiteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Process.Start("https://github.com/amitbagaria/PuttyWrap/");
        }

        private void helpToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (File.Exists(Application.StartupPath + @"\PuttyWrap.chm"))
            {
                Process.Start(Application.StartupPath + @"\PuttyWrap.chm");
            }
            else
            {
                ...
                {
                    Process.Start("https://github.com/amitbagaria/PuttyWrap/wiki");
                }
            }
        }
        ...
        {
            if (MessageBox.Show("Exit PuttyWrap?", "Confirm Exit", MessageBoxButtons.OKCancel, MessageBoxIcon.Exclamation) == DialogResult.OK)
            {
                ...
                Classes.Database.SetKeyStatic("main_form_location_x", this.Location.X.ToString());
                Classes.Database.SetKeyStatic("main_form_location_y", this.Location.Y.ToString());                              
        ...
        void checkForUpdate(bool automatic)
        {
        	...
        	Classes.Database d = new PuttyWrap.Classes.Database();
        	d.Open();
			...
			if (performCheck && automatic)
			{
	            try
	            {
	                string url = "https://github.com/amitbagaria/PuttyWrap/raw/master/VERSION";
	                string text = "";
	                ...
	                {
	                    if (MessageBox.Show("There is a new version available. Would you like to open the dicussion page to download it?", "PuttyWrap - New version available!", MessageBoxButtons.YesNo) == DialogResult.Yes)
	                    {
	                ...
	                else
	                {
	                    if (!automatic)
	                    {
	                        MessageBox.Show("No new version available.", "PuttyWrap");
	                    }
	                }
	            }
	            catch (WebException e)
	            {
	                if (!automatic)
	                {
	                    MessageBox.Show("Error while checking for updates: " + e.Message, "PuttyWrap");
	                }
	            }
			}
        }
        ...
        {
        	Process.Start("http://mbimreit-techforum/");
        }
        ...
        {
            if (MessageBox.Show("Do you want to copy all sessions from PuTTY? This may overwrite identically named sessions in PuttyWrap!", "PuttyWrap", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
	    ...
	    private void firstTimeAutomaticUpdateCheck()
	    {
			...
			{
				bool enabled = false;
				if (MessageBox.Show("Do you wish to enable automatic update checks?", "PuttyWrap", MessageBoxButtons.YesNo) == DialogResult.Yes)
				{
					enabled = true;
				}
				...
				MessageBox.Show("You may enable/disable automatic update checks by navigating to the File->Settings menu.", "PuttyWrap", MessageBoxButtons.OK);
			}
	    }
        ...
        void FrmPuttyWrapSizeChanged(object sender, EventArgs e)
        {
        }
        
        private void setWindowStateAndSize()
        {
            String formState = Classes.Database.GetStringKey("main_form_state", "Normal");
            if (formState == null || formState == "")
                formState = "Normal";
            this.WindowState = (FormWindowState) Enum.Parse(typeof(FormWindowState), formState);
            ...
            this.DesktopLocation = new Point(Classes.Database.GetIntegerKey("main_form_location_x", 200),
                Classes.Database.GetIntegerKey("main_form_location_y", 200));
        }
        
        void FrmPuttyWrapFormClosing(object sender, FormClosingEventArgs e)
        {
			Classes.Database.SetKeyStatic("main_form_state", this.WindowState.ToString());
			Classes.Database.SetKeyStatic("main_form_height", this.Height.ToString());
			Classes.Database.SetKeyStatic("main_form_width", this.Width.ToString());
