    ...
    public class SessionData
    {
        ...
        public void SaveToRegistry()
        {
                ...
                && this.Port >= 0)
            {
                ...
                RegistryKey key = Registry.CurrentUser.OpenSubKey(@"Software\Morgan Stanley\PuttyWrap\Sessions\" + this.SessionName, true);
                if (key == null)
                {
                    key = Registry.CurrentUser.CreateSubKey(@"Software\Morgan Stanley\PuttyWrap\Sessions\" + this.SessionName);
                }
        ...
        internal void RegistryRemove(string sessionName)
        {
            ...
            {
                RegistryKey key = Registry.CurrentUser.OpenSubKey(@"Software\Morgan Stanley\PuttyWrap\Sessions", true);
                try
                {
